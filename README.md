...
A high-performance hook for AI coding agents that blocks destructive commands before they execute, protecting your work from accidental deletion across Claude Code, Codex CLI, Gemini CLI, Copilot CLI, VS Code Copilot Chat, Cursor, Hermes Agent, Grok (xAI), Posit Assistant, Oh My Pi, Kiro, and related tools.
...
- **Kiro (`kiro-cli`):** Native `preToolUse` hook support. Kiro runs a hook before its shell tool (`execute_bash`, aliases `execute_cmd` / `shell`), piping `{"hook_event_name":"preToolUse","cwd":"…","tool_name":"execute_bash","tool_input":{"command":"…"}}` to the hook's stdin. dcg recognizes the envelope from those tool names (no other supported agent uses them) and identifies the agent from the `KIRO_SESSION_ID` env var Kiro sets in every hook subprocess. Kiro is the one supported agent whose block channel is the **process exit code, not stdout JSON**: exit 2 blocks the tool and returns STDERR to the model as the reason, exit 0 allows, and stdout on exit 0 is captured but never shown. dcg therefore emits an empty stdout, writes the reason to stderr, and exits 2 on a denial (every other protocol blocks via a stdout-JSON decision on exit 0). **Install with `dcg install --kiro --agent <name>`.** Unlike every other agent, Kiro has no global hook file — hooks live inside each agent config (`~/.kiro/agents/<name>.json`), so `--agent <name>` is required and names a user/global agent. dcg merges a marker-owned `dcg-guard` entry into that agent's `hooks` — matching whichever of Kiro's two hook formats the agent already uses (the trigger-keyed **object** form `hooks.preToolUse[]`, or the flat **array** form with `trigger`/`action`) — preserving every other field and hook; Kiro hot-reloads agent configs, so the guard applies without restarting a running session. Built-in agents (`kiro_default`/`kiro_guide`/`kiro_planner`) cannot be edited and are rejected; a missing agent errors with a `kiro-cli agent create` hint, and running `dcg install --kiro` with no `--agent` lists the available user agents. Remove it with `dcg uninstall --kiro --agent <name>` (only the marker-owned entry is removed). The installers detect Kiro and print this guidance rather than auto-configuring, since the target agent must be chosen. Kiro's own `shell.deniedCommands` / `denyByDefault` config is a complementary, hook-independent guardrail.
